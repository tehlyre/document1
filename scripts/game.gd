...
func debug(debugger):
	for i in range(0, len($container/Room/enemies.get_children())):
		...
		debugger.display_variable("Enemy "+str(i+1)+":", i*4)
		debugger.display_variable(j[i].getmovestate(), i*4+1)
		debugger.display_variable(j[i].stopped, i*4+2)
		debugger.display_variable(" ", i*4+3)
...
var debugger = preload("res://scenes/debug_window.tscn")
...
func _ready():
	...
	d = debugger.instantiate()
	add_child(d)
	d.position = Vector2(20,100)
	d.ready_labels(12)
...
func _on_player_open_chest(i : int):
...
func _on_player_death(_died : bool):
